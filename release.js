...
const RESOURCE_PATH = path.resolve(__dirname, 'release', 'fivem-appearance');
...
const TYPESCRIPT_BUILD_SRC = path.resolve(__dirname, 'typescript', 'build');
const UI_BUILD_SRC = path.resolve(__dirname, 'ui', 'build');
const LOCALES_SRC = path.resolve(__dirname, 'locales');

const TYPESCRIPT_BUILD_DEST = path.resolve(RESOURCE_PATH, 'typescript', 'build');
...
async function execute() {
  ...
  for (const file of FILES) {
    try {
      await fs.copyFile(path.join(__dirname, file), path.join(RESOURCE_PATH, file));
    } catch(err) {
      console.log(err);
    }
  }
