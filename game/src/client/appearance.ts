...
import { getPedTattoos, setPedTattoos } from './modules/customization';
...
export function getPedModel(ped: number): string {
  return pedModelsByHash[GetEntityModel(ped)];
}

export function getPedComponents(ped: number): PedComponent[] {
  const components = [];
...
export function getPedProps(ped: number): PedProp[] {
  const props = [];
...
export function getPedHeadBlend(ped: number): PedHeadBlend {
...
export function getPedHairDecorationType(ped: number): 'male' | 'female' {
...
export function getPedHairDecoration(ped: number, hairStyle: number): HairDecoration {
...
export function setPedAppearance(ped: number, appearance: Omit<PedAppearance, 'model'>): void {
  if (!appearance) return;
